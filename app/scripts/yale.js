...
function newRate() {
  var currentBG = parseFloat(document.getElementById("currentBG").value, 10),
    ...
    timeSinceLastBG = parseFloat(
      document.getElementById("timeSinceLastBG").value,
      10
    ),
  ...
  function giveInstructions() {
    ...
    function applyDelta2X() {
      if (
        (rateOfChangeOfBG > 0 && dripRateDelta2X >= 10) ||
        (rateOfChangeOfBG > 25 && dripRateDelta2X >= 10) ||
        (rateOfChangeOfBG > 50 && dripRateDelta2X >= 10) ||
        (rateOfChangeOfBG > 0 && dripRateDelta2X >= 10) ||
        (rateOfChangeOfBG <= -50 && dripRateDelta2X >= 10) ||
        (rateOfChangeOfBG <= -100 && dripRateDelta2X >= 10)
      ) {
        delta2XAlert = true;
      }
    }
    ...
    function calcB5() {
      ...
      if (rateOfChangeOfBG >= -25 && rateOfChangeOfBG <= 0) {
        return (
          "New Rate: " + (currentRate - dripRateDelta).toString() + " units/hr"
        );
      }

      if (rateOfChangeOfBG < -25) {
        return (
          "STOP INFUSION. Check BG every 30 minutes. When BG &#8805; 100mg/dL, restart drip at " +
          (currentRate * 0.75).toString() +
          " units/hr"
        );
      }
    }
    ...
    function calcB6() {
      ...
      if (rateOfChangeOfBG < -50) {
        return (
          "Hold infusion for 30 minutes, then start infusion at " +
          (currentRate - dripRateDelta2X).toString() +
          " units/hr"
        );
      }
    }
  ...
  if (currentBG >= 60) {
    printMessage(
      "When BG &#8805; 100 mg/dL, wait 1 hour, then restart infusion at 50% of original rate.",
      B3
    );
  }
  ...
  if (currentBG < 60) {
    printMessage(
      "When BG &#8805; 100 mg/dL, wait 1 hour, then restart infusion at " +
        currentRate * 0.5 +
        " units/hr.",
      B3
    );
